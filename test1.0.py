...
MAX_FILES = 10  # 10 = 最多允许加入 10 个 PDF 文件
...
class PdfListWidget(QListWidget):
    def __init__(self, parent_window, ui_cfg):
        ...
        self.setSpacing(self.ui_cfg["LIST_ITEM_SPACING"])  # 文件行与文件行之间的垂直间距像素
        self.setMouseTracking(True)
...
class PdfMergeWindow(QWidget):
    ...
    def init_ui(self):
        ...
        tip_label = QLabel("拖入 1~10 个 PDF；按列表顺序拼接；允许同一文件重复加入；支持拖动调整顺序")
        ...
        self.output_entry = QLineEdit("merged.pdf")
    ...
    def refresh_list(self):
        ...
        actual_item_height = max(
            self.ui_cfg["FILE_ITEM_HEIGHT"],  # 你设定的基础行高
            self.ui_cfg["DELETE_BUTTON_HEIGHT"] + self.ui_cfg["ROW_MARGIN_V"] * 2  # 保证删除按钮不会被裁切
        )

        for i, path in enumerate(self.selected_files, 1):  # 1 = 序号从 1 开始显示
            file_name = os.path.basename(path)

            item = QListWidgetItem()
            item.setData(Qt.UserRole, path)
            item.setSizeHint(QSize(
                100,               # 100 = 基础宽度占位，实际宽度主要由列表控件决定
                actual_item_height # 文件行最终高度像素
            ))
